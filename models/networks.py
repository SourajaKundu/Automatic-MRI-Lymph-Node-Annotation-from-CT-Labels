...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    if netF == 'global_pool':
        net = PoolingF(init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids)
    elif netF == 'reshape':
        net = ReshapeF()
    elif netF == 'sample':
...
# feature is of size (256,256) and pooling is done globally instead on some patches, so there exizts sme issue regarding the size
class PoolingF(nn.Module):
    def __init__(self, init_type='normal', init_gain=0.02, gpu_ids=[]):
        ...
        print(f'length of gpu id passed: {len(self.gpu_ids)}')

    def create_weight_function(self, patch_size=(50, 100), threshold=200, boost_factor=0.6):
        print(f'length of gpu id: {len(self.gpu_ids)}')
        return CustomWeightFunction(patch_size, threshold, boost_factor, gpu_ids=self.gpu_ids)

    def create_weighted_layer_pool(self):
        weight_function = self.create_weight_function()
        return WeightedLayerPool2d(kernel_size=self.kernel_size, weight_function=weight_function, gpu_ids=self.gpu_ids)

    def forward(self, feats, num_patches=64, patch_ids=None):
        print(f'length of gpu id before calling: {len(self.gpu_ids)}')
        return_ids = []
        return_feats = []

        weighted_pool = self.create_weighted_layer_pool()
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if self.gpu_ids:
                feat_reshape = feat_reshape.to(self.gpu_ids[0])

            if num_patches > 0:
                if patch_ids is not None:
                    patch_id = patch_ids[feat_id]
                else:
                    patch_id = torch.randperm(feat_reshape.shape[1], device=feat_reshape.device)
                    patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]

                # Use the weighted pooling on specific patches
                x_sample = feat_reshape[:, patch_id, :].flatten(0, 1)
                x_sample = weighted_pool(x_sample)
            else:
                # Use the weighted pooling on the entire feature map
                x_sample = weighted_pool(feat_reshape.flatten(0, 1))

            if num_patches == 0:
                x_sample = x_sample.view(H, W)

            return_ids.append(patch_id)
            x_sample = nn.functional.normalize(x_sample, dim=-1)
            return_feats.append(x_sample)

        return return_feats, return_ids

class CustomWeightFunction(nn.Module):
    def __init__(self, patch_size=(50, 100), threshold=200, boost_factor=0.6, gpu_ids=[]):
        super(CustomWeightFunction, self).__init__()
    ...
    def forward(self, x):
        # Assuming x has shape (H, W) where H is height, W is width
        H, W = x.size()

        # Move tensor to GPU if specified in gpu_ids
        print(f'length of gpu ids: {len(self.gpu_ids)}')
        if len(self.gpu_ids):
            x = x.to(self.gpu_ids[0])

        # Create a mask for pixels with color values greater than the threshold
        color_mask = (x > self.threshold).float()
        color_mask = color_mask.to(x.device)  # Corrected line

        # Create a boost mask for the small patch region
        boost_mask = torch.zeros((H, W))
        boost_mask[H // 4:H // 4 + self.patch_size[0], W // 4:W // 4 + self.patch_size[1]] = 1.0
        boost_mask = boost_mask.to(x.device)  # Corrected line

        # Combine the color mask and boost mask
        combined_mask = color_mask + self.boost_factor * boost_mask

        # Normalize the weights
        normalized_weights = combined_mask / combined_mask.sum()

        return normalized_weights

class WeightedLayerPool2d(nn.Module):
    def __init__(self, kernel_size, weight_function, gpu_ids=[]):
        super(WeightedLayerPool2d, self).__init__()
    ...
    def forward(self, x):
        ...
        x = x.view(1, 1, H, W)  # Convert the 2D tensor to 4D by adding batch and channel dimensions

        # Move tensors to GPU if specified in gpu_ids
        if self.gpu_ids:
            x = x.to(self.gpu_ids[0])
            weight = weight.to(self.gpu_ids[0])

        x = x * weight
        x = nn.functional.adaptive_avg_pool2d(x, self.kernel_size)
...
class StridedConvF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, x in enumerate(feats):
            C, H = x.shape[1], x.shape[2]
            #print(f"Input shape (x) is {x.shape}")
            key = '%d_%d' % (C, H)
            ...
            if num_patches > 0:
                if patch_ids is not None:
                    ...
                    patch_id = torch.randperm(x.shape[2] * x.shape[3], device=x.device)
                    patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]

                x_sample = x[:, :, patch_id, :].flatten(0, 1)
            else:
                x_sample = x.flatten(2)

            return_ids.append(patch_id)
            x_sample = self.l2_norm(x_sample)

            if num_patches == 0:
                x_sample = x_sample.view(x.shape[2], x.shape[3])
...
class SelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query):
        ...
        if query_len == key_len:
            mask = torch.ones(energy.shape[-2:])
...
class PatchSampleF(nn.Module):
    def __init__(self, use_mlp=False, use_self_attention=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        super(PatchSampleFfinal, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.use_self_attention = use_self_attention  # New parameter
        self.nc = nc
        self.mlp_init = False
        self.self_attn_init = False  # New parameter
        self.init_type = init_type
        self.init_gain = init_gain
        self.gpu_ids = gpu_ids

    def create_mlp_orig(self, feats):
    ...
    def create_self_attention(self, feats):
        for attn_id, feat in enumerate(feats):
            input_nc = feat.shape[1]
            attn = SelfAttention(input_nc, heads=8)  # Adjust heads as needed
            if len(self.gpu_ids) > 0:
                attn.cuda()
            setattr(self, 'self_attn_%d' % attn_id, attn)
        init_net(self, self.init_type, self.init_gain, self.gpu_ids)
        self.self_attn_init = True
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        if self.use_self_attention and not self.self_attn_init:
            self.create_self_attention(feats)
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if self.use_self_attention:
                attn = getattr(self, 'self_attn_%d' % feat_id)
                x_sample = attn(x_sample, x_sample, x_sample)
...
class PatchSampleFDFMIR(nn.Module):
...
class PatchSampleF_ROI(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        super(PatchSampleF, self).__init__()
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            # Extract color information (assuming RGB channels)
            color_info = feat_reshape[:, :3]

            # Identify regions of interest based on color criteria (e.g., R or G or B > 0.8)
            roi_mask = (color_info.max(dim=1).values > 0.8).float()

            # Apply patch sampling to the identified ROIs
            roi_indices = roi_mask.nonzero(as_tuple=True)
            patch_indices = torch.randperm(len(roi_indices[0]))[:num_patches]

            # Use the identified ROIs for patch sampling
            if num_patches > 0:
                x_sample = feat_reshape[:, roi_indices[0][patch_indices], :].flatten(0, 1)
            else:
                x_sample = feat_reshape
            ...
            return_ids.append(patch_indices)
            x_sample = self.l2norm(x_sample)
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            feat = input
            #print("Input shape of generator ",feat.shape)
            feats = []
            for layer_id, layer in enumerate(self.model):
                #print(layer_id, layer)
                feat = layer(feat)
                #print(f"Shape after layer {layer_id} is {feat.shape}")
