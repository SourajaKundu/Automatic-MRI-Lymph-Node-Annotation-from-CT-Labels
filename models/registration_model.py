...
class SSIMLoss:
    ...
    def __call__(self, real_image, generated_image):
        ...
        ssim_value = self.ssim_metric(generated_image, real_image)

        # SSIM loss is 1 - SSIM, as we want to minimize the dissimilarity
        loss = 1.0 - ssim_value
...
def open_image_to_torch(path, size):
    ...
    transforms.append(Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]))  # Assuming RGB images
...
class SuperResolutionLoss(nn.Module):
    def __init__(self, crop_size=256):
        ...
        self.superresolution_model = Net(3, 12).cuda()
        self.crop_size = crop_size

    def forward(self, realB, regA):
        # Crop the central region
        crop_margin = (512 - self.crop_size) // 2
        cropped_realB = realB[:, :, crop_margin:crop_margin+self.crop_size, crop_margin:crop_margin+self.crop_size]
        cropped_regA = regA[:, :, crop_margin:crop_margin+self.crop_size, crop_margin:crop_margin+self.crop_size]

        # Pass through superresolution network
        superresolved_realB = self.superresolution_model(realB)
        superresolved_regA = self.superresolution_model(regA)

        # Calculate L2 loss
        l2_loss = nn.MSELoss()(superresolved_realB, superresolved_regA)

        # Take the square root to represent as a determinant
        determinant_loss = torch.sqrt(l2_loss)

        return determinant_loss.item()  # Convert tensor to scalar
...
class REGISTRATIONModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--netF', type=str, default='mlp_sample', choices=['sample', 'strided_conv', 'reshape', 'mlp_sample', 'global_pool'], help='how to downsample the feature map')
    ...
    def optimize_parameters(self):
        ...
        #print(f'Shape of real MRI: {self.real_B.shape}')
        #print(f'Shape of registered MRI: {self.regA.shape}')
        ...
        ssim_loss_calculator = SSIMLoss()
        ssimloss_value = ssim_loss_calculator(self.real_B, self.regA)
        
        super_resolution_loss_instance = SuperResolutionLoss()
        loss_super_resolution = super_resolution_loss_instance(self.real_B, self.regA)

        self.loss_local = self.calculate_NCE_loss(self.real_B, y_output[0])*0.25
        self.loss_R = self.calculate_L1_loss(y_pred[0],self.real_B, mask=mask)*1.0 + self.calculate_L1_loss(self.idt_B, y_pred[0], mask=mask2)*1.0 + self.loss_local*1.0 + loss_super_resolution  
        self.loss_smooth = smooothing_loss(y_pred[1]) * 0.25
        all_G_loss = self.loss_R + self.loss_G + self.loss_smooth 
        all_G_loss.backward()
    ...
    def forward(self):
        ...
        #print("Shape of real initial ", self.real.shape)
        ...
        #print("Shape of real and fake ", self.real.shape, self.fake.shape)
        self.fake_B = self.fake[:self.real_A.size(0)]
        #print("Shape of fakeB initial ", self.fake_B.shape)
        #if self.opt.nce_idt:
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_NCE > 0.0:
            #print("Shape of fakeB ",self.fake_B.shape)
    ...
    def calculate_L2_loss(self, src, tgt, mask):
        diff = (src - tgt)**2
        if mask is None:
            return torch.mean(diff)
        elif torch.sum(mask) == 0:
