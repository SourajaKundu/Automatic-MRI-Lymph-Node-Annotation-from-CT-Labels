...
class Net(torch.nn.Module):
    def __init__(self, in_channels, out_channels, base_filter, upscale_factor=3):
        super(Net, self).__init__()

        self.layers = torch.nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=base_filter, kernel_size=2, stride=2, padding=4, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=base_filter, out_channels=base_filter // 2, kernel_size=3, padding = 11, stride = 1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=base_filter // 2, out_channels=out_channels * (upscale_factor ** 2), kernel_size=2, stride=1, padding=10, bias=True),
    ...
    def forward(self, x):
        for layer_id, layer in enumerate(self.layers):
            x = layer(x)
            #print(f'After layer {layer_id + 1}: {x.shape}')
...
class ModulatedConv2d(nn.Module):
    def __init__(
        ...
        self.superresolution = Net(256, 128, base_filter=512, upscale_factor=3)
    ...
    def forward(self, input, style):
        ...
        if self.upsample:
            ...
            #out=self.superresolution(input)
...
class ConvLayer_noblur(nn.Sequential):
    def __init__(
        self,
        in_channel,
        out_channel,
        ...
        if downsample:
            stride = 2
            self.padding = 1  # You may adjust this based on the desired behavior
...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, blur_kernel=[1, 3, 3, 1], downsample=True, skip_gain=1.0):
        ...
        #self.conv2 = ConvLayer(in_channel, out_channel, kernel_size=3, downsample=downsample, bias=True, activate=True)

        if in_channel != out_channel or downsample:
            self.skip = ConvLayer(
                in_channel, out_channel, 3, downsample=downsample, activate=False, bias=False
            )
        else:
    ...
    def forward(self, input):
        ...
        #print(f"Size of out after conv1: {out.shape}")
        out = self.conv2(out)
        #print(f"Size of out after conv2: {out.shape}")

        skip = self.skip(input)
        #print(f"Size of skip output: {skip.shape}")
